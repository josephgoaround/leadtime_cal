...
- **UI HUD Sync:** HTML 구조와 JS 로직 간의 ID 불일치 해결 (`vol-util`, `vol-bar`, `packed-count`).

## **Current Implementation Plan**
1. **Research:** `planner.html`과 `container3d.js` 간의 데이터 바인딩 확인.
2. **Strategy:** `container3d.js` 내의 불필요한 조건문(`if(summary)`) 제거 및 직접적인 DOM 업데이트 로직 적용.
3. **Execution:** `container3d.js` 수정 및 `blueprint.md` 최신화.
4. **Validation:** 시뮬레이션 실행 시 적재율 및 막대 그래프가 정상적으로 업데이트되는지 확인.
