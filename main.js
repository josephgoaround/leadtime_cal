document.addEventListener('DOMContentLoaded', () => {
    ...
    const seaEdges = [
        ...
        ["srilanka_s", "mumbai_outer"], ["mumbai_exit", "mumbai_outer"], ["srilanka_s", "arabian_sea_mid"], ["mumbai_outer", "arabian_sea_mid"], ["dubai_gate", "arabian_sea_mid"], ["jebel_ali_gate", "dubai_gate"], ["arabian_sea_mid", "bab_el_mandeb"], ["bab_el_mandeb", "red_sea_1"], ["red_sea_1", "red_sea_2"], ["red_sea_2", "red_sea_3"], ["red_sea_3", "suez_s"], ["suez_s", "suez_n"], ["suez_n", "piraeus_exit"], ["suez_n", "med_mid"], ["med_mid", "valencia_exit"], ["med_mid", "gibraltar"], ["srilanka_s", "good_hope"],
    ...
    function findMaritimePath(start, end, redSeaRisk) {
        ...
        while (nodes.size > 0) {
            ...
            seaEdges.forEach(([u, v]) => {
                ...
                if (redSeaRisk && ["red_sea_1", "red_sea_2", "red_sea_3", "bab_el_mandeb", "suez_s"].includes(neighbor)) return;
    ...
    async function fetchSummarizedNews(oHub, dHub) {
        ...
        let newsHtml = `<div class="mt-12 border-t pt-8 animate-fade-in"><h4 class="text-sm font-black text-gray-700 uppercase tracking-widest mb-6">${t.newsTitle}</h4><div class="grid grid-cols-1 md:grid-cols-2 gap-4">`;
        const insights = [
        ...
        insights.forEach(item => {
            newsHtml += `<div class="p-6 bg-white rounded-3xl border border-gray-100 shadow-sm hover:border-indigo-300 transition-all hover:shadow-md"><div class="flex items-center gap-2 mb-2"><span class="w-2 h-2 bg-green-500 rounded-full animate-pulse"></span><h5 class="text-xs font-bold text-indigo-600 uppercase">${item.title}</h5></div><p class="text-xs text-gray-500 leading-relaxed">${item.content}</p></div>`;
        });
        newsHtml += `</div></div>`;
    ...
    function solveRoute(oId, dId, mode, hscode) {
        const o = hubs[oId], d = hubs[dId], isRedSea = document.getElementById('risk-redsea').checked;
        ...
        } else { altDist = getDist(o.coords, d.coords); }
    ...
    async function calculateAndDisplay() {
        ...
        if (formattedCost.length > 10) fontSizeClass = "text-3xl"; else if (formattedCost.length > 7) fontSizeClass = "text-4xl";
        resultContainer.innerHTML = `
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in">
                <div class="p-8 bg-indigo-50 rounded-3xl shadow-xl border border-indigo-100 text-center flex flex-col items-center justify-center min-h-[220px] relative overflow-hidden group">
                ...
                <div class="p-8 bg-green-50 rounded-3xl shadow-xl border border-green-100 text-center flex flex-col items-center justify-center min-h-[220px] relative overflow-hidden group">
                ...
                <div class="p-8 bg-orange-50 rounded-3xl shadow-xl border border-orange-100 text-center flex flex-col items-center justify-center min-h-[220px] relative overflow-hidden group">
        ...
        renderMap(route.path); await fetchSummarizedNews(hubs[oId], hubs[dId]); document.getElementById('executive-actions').classList.remove('hidden');
    }

    function startLiveUpdates() {
        const liveNews = ["Rotterdam Port terminal 4 experiencing 12h berthing delay.", "New zero-emission shipping corridor announced Singapore-LA.", "Panama Canal increases daily transit slots to 24.", "Air cargo rates from Shanghai to Europe stabilize.", "Blockchain integration in Dubai customs reducing clearance time by 20%.", "Heavy storm in North Atlantic causing minor sea route diversions."];
        setInterval(() => {
            ...
            const newItem = document.createElement('span'); newItem.className = "text-indigo-400 animate-pulse"; newItem.textContent = liveNews[randomIndex];
            ticker.appendChild(newItem); if (ticker.children.length > 10) ticker.removeChild(ticker.firstChild);
        }, 15000);
    }
    ...
    function updateUI() {
        ...
        if(resultContainer.innerHTML.includes('indigo-50')) calculateAndDisplay(); populate();
    }
